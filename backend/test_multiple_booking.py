...
async def test_multiple_booking_email():
    ...
    try:
        ...
        if email_sent:
            ...
            print(
                f"   📧 Subject: Multiple Tour Confirmation - {', '.join([unit.unit_id for unit in booked_units])}"
            )
...
async def test_inventory_service():
    ...
    for unit_id in test_units:
        ...
        if unit:
            print(
                f"   ✅ Found {unit_id}: {unit.beds} bed/{unit.baths} bath, {unit.sqft} sq ft, ${unit.rent}/month"
            )
        else:
